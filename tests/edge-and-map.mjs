...
// a board about as wide as the real one
const seed=(n,spread)=>{const persons=[],manual={};
  for(let i=0;i<n;i++){const id='p'+i;
    persons.push({id,name:'Person'+i+' Wide',first:'Person'+i,last:'Wide',middle:'',nickname:'',maiden:'',suffix:'',sex:'unknown',birth:1900+i,docs:[]});
    manual[id]={x:i*spread,y:(i%4)*250};}
...
for(let i=0;i<12;i++){ await drag(340,0); await pg.waitForTimeout(260); }   // shove right, repeatedly
...
ok('the blank margin at the edge is roomy but not endless', edge.emptyLeft<2600 && edge.emptyLeft>150,
   JSON.stringify({blankBoardUnits:Math.round(edge.emptyLeft)}));
// the same going up
for(let i=0;i<10;i++){ await drag(0,340); await pg.waitForTimeout(240); }
await pg.waitForTimeout(1600);
...
await pg.close(); pg=await open(seed(3,140));
...
await pg.close(); pg=await open(seed(40,900));
...
ok('and zooming back in stays over the same spot', Math.abs(nowAt.x-wasAt.x)<400 && Math.abs(nowAt.y-wasAt.y)<400,
